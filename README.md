# Carlos Eduardo

Cloud & DevOps Engineer, com foco em governança de nuvem, infraestrutura como código e arquitetura de software orientada a domínio (DDD).

Atuo em ambiente híbrido (on-premises + AWS), conduzindo iniciativas de adoção de nuvem baseadas no AWS Cloud Adoption Framework (CAF), além de projetos pessoais em Terraform, observabilidade e desenvolvimento .NET.

#### Social

<a href="https://www.linkedin.com/in/carlossfb/" target="_blank">
  <img src="https://img.shields.io/static/v1?message=LinkedIn&logo=linkedin&label=&color=a89661&logoColor=white&labelColor=&style=for-the-badge" height="28" alt="linkedin" />
</a>

#### Cloud & Infraestrutura

![AWS](https://img.shields.io/badge/AWS-000?style=for-the-badge&logo=amazon-aws&logoColor=FF9900)&nbsp;
![Azure](https://img.shields.io/badge/Azure-000?style=for-the-badge&logo=microsoft-azure&logoColor=0078D4)&nbsp;
![Terraform](https://img.shields.io/badge/Terraform-000?style=for-the-badge&logo=terraform)&nbsp;
![Docker](https://img.shields.io/badge/Docker-000?style=for-the-badge&logo=docker&logoColor=2496ED)&nbsp;
![Traefik](https://img.shields.io/badge/Traefik-000?style=for-the-badge&logo=traefikproxy&logoColor=24A1C1)&nbsp;
![Linux](https://img.shields.io/badge/-Linux-0D1117?style=for-the-badge&logo=linux&labelColor=0D1117)&nbsp;
![Windows](https://img.shields.io/badge/-Windows-0D1117?style=for-the-badge&logo=windows&labelColor=0D1117)

#### Observabilidade

![Grafana](https://img.shields.io/badge/Grafana-000?style=for-the-badge&logo=Grafana)&nbsp;
![Zabbix](https://img.shields.io/badge/Zabbix-000?style=for-the-badge)&nbsp;
![VictoriaMetrics](https://img.shields.io/badge/VictoriaMetrics-000?style=for-the-badge)

#### Desenvolvimento

![C#](https://img.shields.io/badge/C%23-.NET-a89661?style=for-the-badge&labelColor=000)&nbsp;
![Java](https://img.shields.io/badge/Java-Spring-a89661?style=for-the-badge&labelColor=000)&nbsp;
![PHP](https://img.shields.io/badge/PHP-000?style=for-the-badge&logo=php)

Transito entre stacks conforme o problema pede — o fio condutor é arquitetura (DDD, modularização) e infraestrutura como código, não uma linguagem específica.

#### Em destaque

- **[imagebuilder](https://github.com/carlossfb/imagebuilder)** — Módulos Terraform para criação de golden AMIs via EC2 Image Builder (Ubuntu 22.04, us-west-2).
- **NeoParking-DDD-dotnet** — Monólito modular em .NET 8 aplicando DDD, com bounded contexts desacoplados via eventos de integração (Shared Kernel) e padrão Outbox.

<!--![snake gif](https://github.com/carlossfb/carlossfb/blob/output/github-contribution-grid-snake.svg) -->
